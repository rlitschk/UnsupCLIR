...
def tokenize(text, language, tokenize_fn, exclude_digits=False):
    """
    ...
    for token in tokenize_fn(text): 
        if token not in lang_stopwords and token.lower() not in lang_stopwords and token not in punctuation and len(token) > 1:
            if exclude_digits:
...
def text2vec_idf_sum(text, tokenize_fn, language='', dim=300):
    """
    ...
    :return:
    """
    return text2vec_sum(text, tokenize_fn=tokenize_fn, language=language, dim=dim)


def text2vec_sum(text, tokenize_fn, language='', dim=300):
    """
    ...
    _all = 0
    _unique = set()
    for token in tokenize(text, tokenize_fn=tokenize_fn, language=language):
        _all += 1
        _unique.add(token)
...
def _map_func(id_text, language, tokenize_fn):
    """
    ...
    """
    _id, doc = id_text
    doc = tokenize(clean(doc), tokenize_fn=tokenize_fn, language=language)
    # result = list(set(doc))
...
def compute_idf_weights(text, language, processes, tokenize_fn):
    """
    ...
    _map_func_language = partial(_map_func, tokenize_fn=tokenize_fn, language=language)
...
def create_text_representations(language, id_text, emb, tokenize_fn, method=text2vec_sum, processes = 40, idf_weighing = False):
    """
    ...
    if idf_weighing:
        ...
        # compute IDF weights
        idf_weights = compute_idf_weights(id_text, language, processes=processes, tokenize_fn=tokenize_fn)
    ...
    pool = Pool(processes=processes)
    partial_method = partial(method, language=language, tokenize_fn=tokenize_fn)
